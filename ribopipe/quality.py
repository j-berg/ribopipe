...
def quality(df, plotdir, type):
    ...
    if len(sample_list)/2 <= 2:
        ...
        fig_size = (15,(10*(int(len(sample_list)/4))))
...
def meta_run(args):
    ...
    if 'type' in args_dict and args_dict['type'] == 'riboseq':
        ...
        os.system('metagene count -q ' + dir_dict['reference'] + 'transcripts_cds_start_200_rois.txt ' + dir_dict['plastdir'] + file[:-4] + '_periodicity --count_files ' + dir_dict['bamdir'] + file + ' --fiveprime --offset 14 --normalize_over 30 200 --min_counts 50 --cmap Blues --title ' + file[:-4])
